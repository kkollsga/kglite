...
def build_users(g, n=30000, edges_per=3):
    ...
    g.cypher(
        "UNWIND $r AS r CREATE (:User {id:r.id, age:r.age, city:r.city, active:r.active, a:r.a, b:r.b, c:r.c, d:r.d})",
        params={"r": rows},
    )
    pairs = [{"s": i, "t": (i + k + 1) % n} for i in range(n) for k in range(edges_per)]
    g.cypher("UNWIND $p AS e MATCH (a:User {id:e.s}), (b:User {id:e.t}) CREATE (a)-[:KNOWS]->(b)", params={"p": pairs})
    return g
...
record(
    "M7_where_distinct_orderby",
    bench(
        lambda: G.cypher("MATCH (u:User) WHERE u.age > 40 RETURN DISTINCT u.city AS c ORDER BY c").to_list(), rounds=30
    ),
...
record(
    "M3_two_hop_fanout",
    bench(lambda: G.cypher("MATCH (a:User)-[:KNOWS]->(b:User)-[:KNOWS]->(c:User) RETURN count(*) AS n"), rounds=10),
...
for k in (2, 3, 4):
    record(
        f"M9_varlen_depth_{k}",
        bench(
            lambda k=k: G.cypher(f"MATCH (a:User {{id:0}})-[:KNOWS*1..{k}]-(b) RETURN count(DISTINCT b) AS n"),
            rounds=20,
        ),
        f"-[*1..{k}]- from one node",
    )
...
ndv_q = "MATCH (u:User {active:true})-[:KNOWS]->(v:User {city:'c0'}) RETURN count(*) AS n"
