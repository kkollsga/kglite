...
def test_bare_save_round_trips(tmp_path):
    p = str(tmp_path / "app.kgl")
    g = kglite.open(p, encoding="utf-8")
    g.cypher("CREATE (:Person {id: 1, name: 'Alice'})")
    g.save()
    g = kglite.open(p, encoding="utf-8")
    g.cypher("CREATE (:Person {id: 2, name: 'Bob'})")
    g.save()
    assert kglite.open(p, encoding="utf-8").cypher("MATCH (p) RETURN count(*) AS c").scalar() == 2


def test_save_as_updates_remembered_path(tmp_path):
    p1 = str(tmp_path / "a.kgl")
    p2 = str(tmp_path / "b.kgl")
    g = kglite.open(p1, encoding="utf-8")
    ...
    g.save()
    assert kglite.open(p2, encoding="utf-8").cypher("MATCH (p) RETURN count(*) AS c").scalar() == 2
...
def test_load_remembers_path(tmp_path):
    p = str(tmp_path / "app.kgl")
    kglite.open(p, encoding="utf-8").save()  # create empty file
    g = kglite.load(p)
    g.cypher("CREATE (:Person {id: 1})")
    g.save()  # bare save works because load() remembered the path
    assert kglite.open(p, encoding="utf-8").cypher("MATCH (p) RETURN count(*) AS c").scalar() == 1


def test_context_manager_autosaves_on_clean_exit(tmp_path):
    p = str(tmp_path / "app.kgl")
    with kglite.open(p, encoding="utf-8") as g:
        g.cypher("CREATE (:Person {id: 1, name: 'Alice'})")
...
def test_non_durable_context_manager_discards_on_exception(tmp_path):
    ...
    p = str(tmp_path / "app.kgl")
    with kglite.open(p, durable=False, encoding="utf-8") as g:
        g.cypher("CREATE (:Person {id: 1})")
    ...
    assert kglite.open(p, durable=False, encoding="utf-8").cypher("MATCH (p) RETURN count(*) AS c").scalar() == 1


def test_context_manager_does_not_suppress_exception(tmp_path):
    p = str(tmp_path / "app.kgl")
    with pytest.raises(ValueError):
        with kglite.open(p, encoding="utf-8"):
