...
pub(super) fn write_cypher_overview(xml: &mut String) {
    ...
    xml.push_str("    <item feature=\"Constraints on relationships, and property types outside the accepted set\" note=\"All constraint DDL is node-only: a relationship constraint is rejected. UNIQUE, NOT NULL, NODE KEY and IS :: TYPE are supported and enforced on every write path (Cypher CREATE/MERGE/SET and bulk loads alike). IS :: accepts BOOLEAN, STRING, INTEGER, FLOAT, DATE, LOCAL DATETIME, DURATION, POINT — the type names with an exact value counterpart; LIST&lt;...&gt;, unions, zoned temporal types and decorated forms are rejected rather than approximated. For those, define_schema() plus validate_schema() audits existing data and lock_schema() rejects writes that disagree with a node type's recorded property type.\"/>\n");
