...
pub(super) fn write_cypher_overview(xml: &mut String) {
    ...
    xml.push_str("    <clause name=\"CREATE CONSTRAINT\">Schema DDL, a standalone statement, enforced on every write path. CREATE CONSTRAINT [name] [IF NOT EXISTS] FOR (n:Label) REQUIRE n.prop IS UNIQUE — or REQUIRE (n.a, n.b) IS UNIQUE for a composite tuple, IS NOT NULL for presence, IS NODE KEY for both at once (installed atomically). The Neo4j 4 ASSERT spelling works. Declaring a constraint the existing data already violates is rejected and changes nothing, so deduplicate or populate first. Constraint names ARE stored (unlike index names), so DROP CONSTRAINT by name works. Counters land in last_mutation_stats.constraints_added.</clause>\n");
    ...
    xml.push_str("    <clause name=\"SHOW CONSTRAINTS\">List declared constraints as a read: columns name, type (UNIQUENESS|NODE_KEY|NODE_PROPERTY_EXISTENCE), entityType, labelsOrTypes, properties. Same rows as CALL db.constraints(); use that form when you need YIELD or WHERE.</clause>\n");
