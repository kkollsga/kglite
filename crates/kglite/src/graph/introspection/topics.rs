...
pub(super) fn write_topic_functions(xml: &mut String) {
    ...
    xml.push_str("    <group name=\"semantic\">text_score(n, 'col', 'query'|[0.1,0.2,...] [, metric] [, {exact:true}]) — similarity score; exact:true bypasses HNSW, otherwise a compatible top-k may use it; a list query is scored as your query vector, a string query is embedded via set_embedder() (metrics: 'cosine', 'poincare', 'dot_product', 'euclidean'); embedding_norm(n, 'col') — L2 norm of embedding vector (hierarchy depth in Poincaré space, 0=root, ~1=leaf)</group>\n");
