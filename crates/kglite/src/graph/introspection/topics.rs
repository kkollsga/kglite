...
pub(super) fn write_cypher_overview(xml: &mut String) {
    ...
    xml.push_str("    <item feature=\"Multi-label nodes\" note=\"Single label per node by design (not a TODO). labels(n) returns a 1-element list; 'X' IN labels(n) works. Change a node's type via SET n.type = 'NewType'. Rationale + scope: docs/explanation/multi-label-rationale.md\"/>\n");
