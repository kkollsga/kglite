...
pub(super) fn write_topic_functions(xml: &mut String) {
    ...
    xml.push_str("    <group name=\"list\">reduce(acc = init, x IN list | body) — fold accumulator over list; any/all/none/single(x IN list WHERE pred); [x IN list WHERE pred | map_expr] — comprehension; [(n)-[:R]->(m) WHERE pred | m.name] — pattern comprehension, one element per match</group>\n");
...
pub(super) fn write_fluent_topic_loading(xml: &mut String) {
    ...
    xml.push_str("    <methods>\n");
    xml.push_str("      <m sig=\"add_nodes(data, node_type, unique_id_field, node_title_field=None, columns=None, conflict_handling=None, column_types=None, timeseries=None, labels=None, git_sha=None, modified_by=None, on_invalid='warn', convention=None)\">Load nodes. conflict_handling: 'update'|'replace'|'skip'|'preserve'|'sum'. on_invalid: 'warn' (default, skip unusable rows and warn), 'error' (refuse the whole call, nothing written), 'skip' (silent). Provenance is stamped on auto_timestamp types. A type's unique_id_field/node_title_field is fixed once it has nodes: naming a different one raises and writes nothing. column_types {col: 'validFrom'|'validTo'} declares the validity interval (convention 'closed' default, or 'half_open'), validated before the write.</m>\n");
    xml.push_str("      <m sig=\"add_relationships(data, connection_type, source_type, source_id_field, target_type, target_id_field, columns=None, skip_columns=None, conflict_handling=None, query=None, extra_properties=None, git_sha=None, modified_by=None, on_invalid='warn', convention=None)\">Load edges from DataFrame or a read query. on_invalid controls rows with a null endpoint id, as in add_nodes. Provenance is stamped on auto_timestamp edge types. validFrom/validTo column_types declare the type's validity interval as in add_nodes; on a declared type a row with a new from bound is a parallel edge, not a merge.</m>\n");
