...
pub(super) fn write_topic_functions(xml: &mut String) {
    ...
    xml.push_str("    <group name=\"graph\">size(list), length(path), id(node), labels(node), type(rel), coalesce(expr,...) — first non-null, range(start,end[,step]) — checked inclusive list subject to the max_work_units budget and a 256 MiB materialization ceiling, keys(node), properties(node)/properties(rel) — full property map, start_node(rel)/end_node(rel) — endpoints</group>\n");
