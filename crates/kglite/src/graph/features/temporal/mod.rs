...
use eval::{BoundSide, Instant, IntervalConvention, TemporalError};
...
/// The first config whose `valid_from` or `valid_to` field the edge carries;
/// an edge carrying neither is not temporal.
...
/// [`is_temporally_valid`] under the first config the edge carries fields
/// for; `true` when it carries none.
pub fn is_temporally_valid_multi(
    properties: &[(InternedKey, Value)],
    configs: &[TemporalConfig],
    reference: &NaiveDate,
) -> Result<bool, String> {
    match matching_config(properties, configs) {
