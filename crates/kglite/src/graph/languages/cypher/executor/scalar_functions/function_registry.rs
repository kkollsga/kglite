...
pub(crate) const FUNCTIONS: &[FunctionSpec] = &[
    ...
    FunctionSpec {
        ...
        aliases: &[],
        category: "utility",
        description: "Whether an entity's validity interval covers a date: the type's declared bounds and convention, or the named bounds (closed unless the type's declaration names the same pair); null bounds are open",
        signature: "valid_at(entity :: NODE | RELATIONSHIP, date :: DATE | LOCAL DATETIME | STRING [, fromField :: STRING, toField :: STRING]) :: BOOLEAN",
    },
    FunctionSpec {
        name: "valid_during",
        aliases: &[],
        category: "utility",
        description: "Whether an entity's validity interval overlaps a date range: the type's declared bounds and convention, or the named bounds (closed unless the type's declaration names the same pair); null bounds are open",
