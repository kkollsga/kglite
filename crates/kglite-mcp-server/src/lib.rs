...
fn mode_change_handler(mode: &Mode, graph_state: &GraphState) -> Option<watch::ChangeHandler> {
    match mode {
        Mode::Watch { .. } => {
...
fn resolved_mode_watch_root(mode: &Mode) -> Result<Option<PathBuf>> {
    let root = match mode {
        Mode::Watch { dir } => dir,
        Mode::LocalWorkspace {
            root, watch: true, ..
