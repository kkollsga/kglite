...
def build_graph(
...
) -> tuple[kglite.KnowledgeGraph, list[list[str]], dict[str, set[str]], float]:
    """Build the 500k Star / ~3.4M-3.9M HYPERLANE graph matching NornicDB's
    topology exactly. Returns (graph, sector_members, undirected_adj, build_seconds).
    ...
    `storage` is one of "memory" (default), "mapped" (mmap-backed columns),
    or "disk" (CSR + mmap, requires `disk_path`).
    """
    print(
    ...
    if storage == "memory":
        ...
        if disk_path is None:
            raise ValueError("storage='disk' requires disk_path")
    ...
    return g, sector_members, adj, total
...
def run_one(storage: str, disk_path: str | None = None) -> dict:
    """Build + bench for one storage mode. Returns
    {build_seconds, results: {depth: {min, median, p95, max, n}}}."""
    g, sector_members, adj, build_seconds = build_graph(storage=storage, disk_path=disk_path)
    ...
    return {"build_seconds": build_seconds, "results": results}


def print_three_way_table(by_mode: dict[str, dict]) -> None:
    """Final 3-mode + NornicDB comparison."""
    print(f"\n{'=' * 110}")
    print("FINAL COMPARISON — kglite (Apple M4) vs NornicDB published medians (Apple M3 Max)")
    print(f"{'=' * 110}")

    # Build-time row
    print(f"\n{'Metric':<22}{'kglite memory':<18}{'kglite mapped':<18}{'kglite disk':<18}{'NornicDB':<14}")
    print("-" * 110)
    mem_b = by_mode.get("memory", {}).get("build_seconds")
    map_b = by_mode.get("mapped", {}).get("build_seconds")
    disk_b = by_mode.get("disk", {}).get("build_seconds")
    print(
        f"{'BUILD (500k+4M)':<22}"
        f"{fmt_t(mem_b) if mem_b else '—':<18}"
        f"{fmt_t(map_b) if map_b else '—':<18}"
        f"{fmt_t(disk_b) if disk_b else '—':<18}"
        f"{'3 m 32 s':<14}"
    )

    # Depth rows — use union of depths across modes
    all_depths = set()
    for m in by_mode.values():
        all_depths.update(m.get("results", {}).keys())
    headline_depths = sorted(d for d in all_depths if d in (1, 2, 5, 10, 20, 30, 40, 60))
    if not headline_depths:
        headline_depths = sorted(all_depths)

    print(f"\n{'Depth (median)':<22}{'kglite memory':<18}{'kglite mapped':<18}{'kglite disk':<18}{'NornicDB':<14}")
    print("-" * 110)
    for d in headline_depths:

        def cell(mode):
            r = by_mode.get(mode, {}).get("results", {}).get(d)
            return fmt_t(r["median"]) if r else "—"
        ...
        print(
            ...
            f"{cell('mapped'):<18}"
            f"{cell('disk'):<18}"
            f"{(fmt_t(nornic) if nornic else '—'):<14}"
        )


def main() -> None:
    ap = argparse.ArgumentParser(
        description="NornicDB shortestPath benchmark on kglite. Defaults to running all 3 storage modes.",
    )
    ap.add_argument(
        "--storage",
        choices=["memory", "mapped", "disk", "all"],
        default="all",
        help="Storage mode to bench (default: all three)",
    )
    args = ap.parse_args()

    modes = ["memory", "mapped", "disk"] if args.storage == "all" else [args.storage]
    by_mode: dict[str, dict] = {}

    with tempfile.TemporaryDirectory(prefix="nornic_bench_") as tmp:
        for mode in modes:
            print(f"\n{'#' * 80}\n#  STORAGE = {mode.upper()}\n{'#' * 80}")
            disk_path = str(Path(tmp) / f"disk_{mode}") if mode == "disk" else None
            try:
                by_mode[mode] = run_one(mode, disk_path=disk_path)
            except Exception as exc:  # noqa: BLE001
                print(f"  {mode} FAILED: {type(exc).__name__}: {exc}", file=sys.stderr)
                by_mode[mode] = {"build_seconds": None, "results": {}, "error": str(exc)}

    if len(modes) > 1:
        print_three_way_table(by_mode)

    # Save raw results for any downstream analysis.
    out_path = Path(".nornic_bench_results.json")
    out_path.write_text(
        json.dumps(
            {
                mode: {
                    "build_seconds": data.get("build_seconds"),
                    "depth_results": {str(d): r for d, r in (data.get("results") or {}).items()},
