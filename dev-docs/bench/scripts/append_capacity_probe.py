#!/usr/bin/env python3
"""Bounded direct append-capacity attribution; only run while owning the measurement lease.
...
def main() -> None:
    ...
    parser.add_argument("--nodes", type=int, nargs="+", default=[1000, 30000])
    parser.add_argument("--widths", type=int, nargs="+", default=[1, 16])
    ...
    with tempfile.TemporaryDirectory(prefix="append-capacity-", dir=ROOT / "dev-docs/temp") as name:
        package = pathlib.Path(name)
        (package / "target").symlink_to(target.resolve(), target_is_directory=True)
        manifest = f"""[package]
name = "kglite-append-capacity"
version = "0.0.0"
edition = "2021"
[workspace]
[dependencies]
...
"""
        ...
        binaries = [
            ...
            and r.get("target", {}).get("name") == "append_capacity_probe"
        ...
        result = {
            ...
            "buffer_source_sha256": sha(ROOT / "crates/kglite/src/graph/storage/mapped/mmap_vec.rs"),
            "column_source_sha256": sha(ROOT / "crates/kglite/src/graph/storage/column_store/typed_column.rs"),
            "captures": captures,
        }
