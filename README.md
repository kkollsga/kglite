...
| **Ontology** | Declared semantic layer: `is_a` class forest + relationship semantics (`define_ontology`), `SHOW ONTOLOGY`, no-arg validators, `CALL ontology_audit()` scorecard, blueprint data-quality gate. Annotations, not axioms — SKOS in spirit, never OWL. |
